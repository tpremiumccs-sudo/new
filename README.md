...
**Arquitectura** (sin frameworks, sin build):

| Pieza | Qué hace |
|---|---|
| `index.html` + `styles.css` + `app.js` | Toda la interfaz y la lógica de juego (JS vanilla) |
| `net.js` | Sesión (login/registro con PIN) y sincronización del estado con la API |
| `server/server.py` | Servidor único: sirve la app **y** la API (`/api/*`) con SQLite. Solo Python 3 estándar, cero dependencias |
| `deploy/` | Instalación en el servidor (systemd + Cloudflare Tunnel) y deploy en 1 comando |

El progreso **ya no vive en localStorage**: cada alumno tiene cuenta (usuario +
PIN) y su avance se guarda en el servidor, así puede entrar desde el celular,
el laboratorio o su casa y continuar donde iba. El leaderboard y las tareas se
sincronizan solos.

## 🚀 Cómo correrla

```bash
python3 server/server.py          # abre http://localhost:8099
```

Para publicarla en internet (Cloudflare Tunnel + systemd): ver **DEPLOY.md**.
...
- **🗺️ Árbol de lecciones**: los módulos de cada materia forman un camino
  serpenteante de nodos estilo Duolingo, agrupados por parcial y con
  **checkpoints** 🏆 al completar cada grupo. El nodo actual trae la burbuja
  «EMPEZAR»; los completados muestran ✓ y sus estrellas.
- **❤️ Corazones (vidas)**: fallar una pregunta en una lección de módulo cuesta
  1 corazón (máx. 5). Se regeneran solos (1 cada 30 min) y practicar los
  recupera (modo de práctica con ≥80% = +1). Sin corazones no puedes iniciar
  lecciones, pero siempre puedes practicar. Los modos de práctica, exámenes,
  flashcards y juegos no gastan corazones.
...
- **Leaderboard compartido** 🏆 con medallas para el top 3, búsqueda por
  ...
  insignia destacada y el título equipados. **Se sincroniza solo** con el
  servidor (una fila por cuenta: anti-suplantación real); el administrador
  puede editarlo desde su panel.
...
- HTML + CSS + JavaScript vanilla (`index.html` + `styles.css` + `app.js` +
  `net.js`), servidos por `server/server.py` (Python 3 estándar + SQLite).
...
- Progreso persistente **en el servidor** por cuenta (usuario + PIN con hash
  PBKDF2, sesión con cookie HttpOnly). Nada se guarda en localStorage; entra
  desde cualquier dispositivo y sigue donde ibas.
