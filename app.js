...
function heartsBarHTML(){
  let s = '';
  for(let i=0;i<HEART_MAX;i++) s += '<span class="hb'+(i < S.hearts ? '' : ' off')+'">'+(i < S.hearts ? ico('❤️') : ico('🤍'))+'</span>';
...
const NAV_FOR_VIEW = {home:'navAprender',lesson:'navAprender',session:'navAprender',result:'navAprender',guide:'navAprender',modes:'btnModes',memo:'btnModes',escape:'btnModes',flash:'btnModes',blitz:'btnModes',sudden:'btnModes',board:'btnBoard',tasks:'btnTasks',shop:'btnShop',profile:'btnProfile',levels:'btnProfile',stats:'btnProfile'};
function showView(name){
  VIEWS.forEach(v => $('#view-'+v).classList.toggle('hidden', v!==name));
  document.querySelectorAll('.duo-nav-item').forEach(b => b.classList.toggle('is-active', b.id === NAV_FOR_VIEW[name]));
...
function renderHeader(){
  ...
  const statStreakEl = $('#statStreak'); if(statStreakEl) statStreakEl.classList.toggle('zero', !(S.streak > 0));
  ...
  const avSlot = $('#navAvatar') || HDR.btnProfile; avSlot.innerHTML = avatarStack(activeProfile(), 32);
...
function renderHome(){
  ...
  $$('#moduleGrid .lp-guide').forEach(b => b.addEventListener('click', () => { sfx('click'); $('#btnGuide').click(); }));
...
function lessonPathHTML(){
  const colors = ['#58cc02','#ce82ff','#00cd9c','#1cb0f6','#ff9600','#ff4b4b','#2b70c9','#ffc800'];
  ...
  const OFF = [0, 1.3, 2, 1.3, 0, -1.3, -2, -1.3];   // zigzag estilo Duolingo (±45/±70 px)
  let step = 0;
  const nodeHTML = (m, gi) => {
    ...
    return '<div class="lp-row" style="--x:'+x+'">'
      + '<button class="'+cls+'" data-mod="'+m.id+'" style="--mc:'+(locked ? 'var(--surface3)' : st.done ? '#ffc800' : colors[gi % colors.length])+'" '
      ...
      + '<img class="lp-img" src="assets/duo/path-star.svg" alt="">'
  ...
  const checkpointHTML = (g, gi) => {
    ...
    return '<div class="lp-row" style="--x:'+x+'">'
      ...
      + '<img class="lp-img" src="assets/duo/path-review.svg" alt="">'
  ...
  return '<div class="lesson-path">' + groups.map((g, gi) => {
    ...
    return '<div class="lp-banner" style="--mc:'+colors[gi % colors.length]+'">'
      + '<div class="lp-banner-text"><span class="lp-banner-kicker"><img src="assets/duo/back-arrow.svg" alt="">'+esc(subjectById(S.activeSubject).short).toUpperCase()+' · SECCIÓN '+(gi+1)+' · '+done+'/'+total+'</span>'
      + '<span class="lp-banner-name">'+esc(g.name)+'</span></div>'
      + '<button class="lp-guide" type="button" title="Guía de estudio"><img src="assets/duo/guia-icon.svg" alt=""><span>Guía</span></button></div>'
...
function renderSideWidgets(){
  ...
  if(q){
    const m = missionState();
    q.innerHTML = DAILY_MISSIONS.filter(missionActive).slice(0,3).map(d => {
      const cur = Math.min(missionCur(d), d.goal), pct = Math.round(cur/d.goal*100), claimed = !!m.dclaimed[d.id];
      return '<div class="dq-row'+(claimed?' done':'')+'"><div class="dq-ico">'+ico(d.ico)+'</div>'
        + '<div class="dq-body"><b>'+esc(d.name)+'</b><div class="dq-bar"><i style="width:'+pct+'%"></i><span>'+cur+' / '+d.goal+'</span></div></div>'
        + '<img class="dq-chest" src="assets/duo/'+(claimed?'goal-2':'path-chest')+'.svg" alt=""></div>';
    }).join('') || '<p class="q-help">Sin desafíos activos hoy.</p>';
  }
  const lt = $('#leagueText');
  if(lt){
    const ranked = lbRanked(lbLoad()); const pos = ranked.findIndex(s => s.pid === PROFILES.active) + 1;
    lt.innerHTML = pos ? '<b>Vas en el lugar #'+pos+'</b> de '+ranked.length+' en tu grupo' : 'Completa una lección para aparecer en la tabla de tu grupo.';
  }
  const xs = $('#xpStat'); if(xs) xs.textContent = S.xp;
}
(function bindSideWidgets(){
  const ss = $('#statStreak'); if(ss) ss.addEventListener('click', () => $('#btnStreak').click());
  const qa = $('#questsAll'); if(qa) qa.addEventListener('click', () => { sfx('click'); openStreakHub('daily'); });
  $$('[data-open-board]').forEach(b => b.addEventListener('click', () => $('#btnBoard').click()));
})();
...
function renderBoard(){
  ...
  if(typeof renderSideWidgets === 'function') renderSideWidgets();
  ...
  html += '<div class="panel" style="max-width:640px;margin:16px auto"><h3>Ruta de recompensas</h3>'
    + LEVEL_REWARDS.map(r => {
        ...
        return '<div class="lvl-row'+(un?' done':'')+((li.lvl+1)===r.lvl?' next':'')+'">'
          ...
          + '<span class="lvl-state">'+(un?'✓':ico('🔒'))+'</span></div>';
      }).join('')
    + '</div>';
...
function avatarEditorHTML(p, fb){
  ...
  const preview = '<div class="dresser-preview">'
    ...
    + '<div class="tag-list dp-tags">'
      ...
      + '<span class="tag neutral">Racha: '+S.streak+'</span>'
