...
**2. [Verdadero/Falso]** «No paramétrico» significa que la prueba no tiene ningún supuesto.
...
**4. [Relacionar]** Relaciona cada concepto:
...
**5. [Opción múltiple]** El error tipo I consiste en:
...
**6. [Opción múltiple]** Clasifica el dato: «El nivel de satisfacción (bajo, medio, alto)».
...
**7. [Opción múltiple]** Clasifica el dato: «El monto exacto de un siniestro en pesos».
...
**8. [Opción múltiple]** Clasifica el dato: «El tipo de sangre (A, B, AB, O)».
...
**1. [Cálculo numérico]** Para la muestra ordenada {7, 10, 11, 14}, calcula la función de distribución empírica Fₙ(10).
...
**2. [Cálculo numérico]** Para la muestra ordenada {5, 8, 12, 15, 16, 19}, calcula la función de distribución empírica Fₙ(16).

   - **Respuesta:** 0.8333
   - _Procedimiento:_ Observaciones ≤ 16: 5 → n = 6 → Fₙ(16) = 5/6 = 0.8333
   - _Explicación:_ Fₙ(x) = (1/n)·Σ I(Xᵢ ≤ x): cuenta cuántos datos son ≤ x y divide entre n.

**3. [Opción múltiple]** Para la muestra 3, 5, 7, 8, ¿cuánto vale Fₙ(6)?
...
**5. [Relacionar]** Relaciona cada teorema con su convergencia:
...
**6. [Completar]** La función de distribución empírica es Fₙ(x) = (1/n)·Σ I(Xᵢ ≤ ____ ).

   - **Respuesta:** x

**7. [Opción múltiple]** El teorema de Glivenko-Cantelli afirma que:
...
**8. [Cálculo numérico]** Para la muestra ordenada {4, 6, 8, 11, 12}, calcula la función de distribución empírica Fₙ(8).

   - **Respuesta:** 0.6
   - _Procedimiento:_ Observaciones ≤ 8: 3 → n = 5 → Fₙ(8) = 3/5 = 0.6
   - _Explicación:_ Fₙ(x) = (1/n)·Σ I(Xᵢ ≤ x): cuenta cuántos datos son ≤ x y divide entre n.
...
**1. [Cálculo numérico]** Prueba KS de bondad de ajuste a U(0,1). Datos ordenados: {0.34, 0.35, 0.41, 0.43, 0.67, 0.87, 0.96}. Con Fₙ(xᵢ)=i/n y F(x)=x, calcula D = máx|Fₙ(xᵢ)−F(xᵢ)|.

   - **Respuesta:** 0.1971
   - _Procedimiento:_ En cada xᵢ: |i/n − xᵢ| → El máximo ocurre en x = 0.34 → D = 0.1971
   - _Explicación:_ Para la Uniforme(0,1), F(xᵢ)=xᵢ. Se calcula |i/n − xᵢ| en cada punto y se toma el máximo.

**2. [Ordenar pasos]** Ordena el procedimiento de la prueba KS:
...
**3. [Cálculo numérico]** Para estandarizar en KS (parámetros estimados con x̄ y s), calcula z = (xᵢ − x̄)/s con xᵢ=86, x̄=80 y s=6.

   - **Respuesta:** 1
   - _Procedimiento:_ z = (86 − 80)/6 = 1
...
**4. [Cálculo numérico]** Prueba KS de bondad de ajuste a U(0,1). Datos ordenados: {0.07, 0.19, 0.23, 0.43, 0.51, 0.62}. Con Fₙ(xᵢ)=i/n y F(x)=x, calcula D = máx|Fₙ(xᵢ)−F(xᵢ)|.

   - **Respuesta:** 0.38
   - _Procedimiento:_ En cada xᵢ: |i/n − xᵢ| → El máximo ocurre en x = 0.62 → D = 0.38
   - _Explicación:_ Para la Uniforme(0,1), F(xᵢ)=xᵢ. Se calcula |i/n − xᵢ| en cada punto y se toma el máximo.

**5. [Cálculo numérico]** Prueba KS de ajuste a N(50,6) (parámetros conocidos). Datos: {39, 42, 43, 45, 45, 45, 54, 58}. Usa z=(xᵢ−μ)/σ y F(xᵢ)=Φ(z). Calcula D = máx|i/n − F(xᵢ)|.

   - **Respuesta:** 0.5477
   - _Procedimiento:_ Para cada xᵢ: z=(xᵢ−50)/6, F(xᵢ)=Φ(z) → Diferencia máxima en x = 45 → D = 0.5477
...
**6. [Cálculo numérico]** Para estandarizar en KS (parámetros conocidos), calcula z = (xᵢ − μ)/σ con xᵢ=52, μ=50 y σ=4.

   - **Respuesta:** 0.5
   - _Procedimiento:_ z = (52 − 50)/4 = 0.5
   - _Explicación:_ z=(xᵢ−centro)/dispersión. Con parámetros conocidos se usa μ,σ; si se estiman con la muestra, x̄,s (y la tabla de Lilliefors).

**7. [Opción múltiple]** Si la media y la desviación se estiman con la propia muestra, ¿qué tabla de valores críticos se usa?
...
**8. [Cálculo numérico]** Prueba KS de ajuste a N(80,4) (parámetros conocidos). Datos: {75, 80, 85, 85, 86, 87}. Usa z=(xᵢ−μ)/σ y F(xᵢ)=Φ(z). Calcula D = máx|i/n − F(xᵢ)|.

   - **Respuesta:** 0.3944
   - _Procedimiento:_ Para cada xᵢ: z=(xᵢ−80)/4, F(xᵢ)=Φ(z) → Diferencia máxima en x = 85 → D = 0.3944
   - _Explicación:_ Con parámetros conocidos se estandariza z=(xᵢ−μ)/σ y se usa la tabla normal para F(xᵢ)=Φ(z); D es la mayor diferencia con i/n.

**9. [Verdadero/Falso]** En KS, H₀ afirma que los datos sí provienen de la distribución propuesta.

   - **Respuesta:** Verdadero
   - _Explicación:_ H₀: los datos siguen la distribución; H₁: no la siguen.

**10. [Cálculo numérico]** Prueba KS de bondad de ajuste a U(0,1). Datos ordenados: {0.32, 0.49, 0.72, 0.72, 0.75, 0.79, 0.89, 0.92}. Con Fₙ(xᵢ)=i/n y F(x)=x, calcula D = máx|Fₙ(xᵢ)−F(xᵢ)|.

   - **Respuesta:** 0.345
   - _Procedimiento:_ En cada xᵢ: |i/n − xᵢ| → El máximo ocurre en x = 0.72 → D = 0.345
   - _Explicación:_ Para la Uniforme(0,1), F(xᵢ)=xᵢ. Se calcula |i/n − xᵢ| en cada punto y se toma el máximo.
...
**1. [Cálculo numérico]** Para ajustar una exponencial a una muestra de tiempos con n=24 y Σtᵢ=702, estima λ̂ = n / Σtᵢ.

   - **Respuesta:** 0.034188
   - _Procedimiento:_ λ̂ = n/Σtᵢ = 24/702 = 0.034188
   - _Explicación:_ El estimador de máxima verosimilitud de la exponencial es λ̂ = n/Σtᵢ.

**2. [Cálculo numérico]** En una prueba χ² de bondad de ajuste con k=6 clases y p=1 parámetro(s) estimado(s), ¿cuántos grados de libertad hay? (gl = k − p − 1)

   - **Respuesta:** 4
   - _Procedimiento:_ gl = k − p − 1 = 6 − 1 − 1 = 4
   - _Explicación:_ gl = k − p − 1.

**3. [Verdadero/Falso]** Para la exponencial, F(t) = 1 − e^(−λt).

   - **Respuesta:** Verdadero
   - _Explicación:_ Es la función de distribución de la exponencial.

**4. [Relacionar]** Relaciona los símbolos de la prueba χ²:

   - **Respuesta:** Oᵢ ↔ Frecuencia observada · Eᵢ ↔ Frecuencia esperada · k ↔ Número de clases · gl ↔ k − p − 1

**5. [Ordenar pasos]** Ordena la prueba χ² de bondad de ajuste:
...
**6. [Cálculo numérico]** Con frecuencias esperadas Eᵢ=6 en las 4 clases y observadas Oᵢ = [6, 5, 8, 5], calcula χ² = Σ(Oᵢ−Eᵢ)²/Eᵢ.

   - **Respuesta:** 1
   - _Procedimiento:_ (6−6)²/6 + (5−6)²/6 + (8−6)²/6 + (5−6)²/6 → = 1
...
**1. [Cálculo numérico]** Prueba de proporción: n=100, x=26 éxitos, H₀: p=p₀=0.2. Calcula el estadístico z = (p̂ − p₀) / √[p₀(1−p₀)/n]. (Bajo H₀ se usa p₀ en el error estándar.)

   - **Respuesta:** 1.5
   - _Procedimiento:_ p̂ = x/n = 26/100 = 0.26 → EE = √[p₀(1−p₀)/n] = √[0.2·0.8/100] = 0.04 → z = (0.26 − 0.2)/0.04 = 1.5
...
**3. [Cálculo numérico]** Muestra pequeña: X ~ Bin(12, 0.5). Calcula el valor p de cola derecha P(X ≥ 9).

   - **Respuesta:** 0.073
   - _Procedimiento:_ P(X≥9) = Σ desde k=9 hasta 12 de C(12,k)·0.5^12 → = 0.073
   - _Explicación:_ P(X≥x) = Σ_{k=x}^{n} C(n,k) p₀^k (1−p₀)^{n−k}.

**4. [Opción múltiple]** Para una prueba binomial exacta de cola derecha, el valor p es:
...
**5. [Cálculo numérico]** En una muestra de n=80 con x=19 defectuosos, calcula la proporción muestral p̂ = x/n.

   - **Respuesta:** 0.2375
   - _Procedimiento:_ p̂ = 19/80 = 0.2375
...
**1. [Cálculo numérico]** En la prueba de los signos, bajo H₀ el número de signos «+» es X ~ Bin(8, 0.5). Calcula P(X ≤ 0).
...
**2. [Completar]** Bajo H₀, en la prueba de los signos X ~ Bin(n, ____ ).

   - **Respuesta:** 0.5

**3. [Cálculo numérico]** Prueba de los signos para la mediana propuesta 50. Datos: {49, 44, 52, 44, 52, 55, 50, 52, 54, 52, 44}. Tras eliminar los empates con 50, ¿cuál es el tamaño efectivo n?

   - **Respuesta:** 10
   - _Procedimiento:_ Empates con 50: 1 → Menores: 4 · Mayores: 6 → n efectivo = 11 − 1 = 10
   - _Explicación:_ Se descartan los valores iguales a la mediana propuesta; el tamaño efectivo es n menos los empates.

**4. [Opción múltiple]** En datos pareados antes/después, un empate (diferencia = 0) se debe:
...
**5. [Cálculo numérico]** Prueba de los signos para la mediana propuesta 50. Datos: {44, 54, 44, 48, 54, 50, 46, 49, 47}. Tras eliminar los empates con 50, ¿cuál es el tamaño efectivo n?

   - **Respuesta:** 8
   - _Procedimiento:_ Empates con 50: 1 → Menores: 6 · Mayores: 2 → n efectivo = 9 − 1 = 8
   - _Explicación:_ Se descartan los valores iguales a la mediana propuesta; el tamaño efectivo es n menos los empates.

**6. [Verdadero/Falso]** La prueba de los signos usa solo la dirección (signo) de las diferencias, no su magnitud.

   - **Respuesta:** Verdadero
   - _Explicación:_ Solo importa el signo (+/−); la magnitud se ignora.

**7. [Opción múltiple]** El valor que divide la muestra en dos mitades iguales (50% a cada lado) es:
...
**8. [Cálculo numérico]** Prueba de los signos para la mediana propuesta 100. Datos: {104, 94, 97, 95, 99, 101, 98, 103, 100, 98}. Tras eliminar los empates con 100, ¿cuál es el tamaño efectivo n?

   - **Respuesta:** 9
   - _Procedimiento:_ Empates con 100: 1 → Menores: 6 · Mayores: 3 → n efectivo = 10 − 1 = 9
   - _Explicación:_ Se descartan los valores iguales a la mediana propuesta; el tamaño efectivo es n menos los empates.
...
**1. [Opción múltiple]** ¿Qué celdas aportan evidencia sobre el cambio en McNemar?
...
**2. [Cálculo numérico]** Prueba de McNemar. Discordantes B=1, C=3 (B+C=4 exacto). Con X ~ Bin(4, 0.5), calcula P(X ≤ 1) (cola con el menor de B y C).

   - **Respuesta:** 0.3125
   - _Procedimiento:_ menor(B,C) = 1 → P(X ≤ 1) con Bin(4,0.5) = 0.3125
   - _Explicación:_ Con B+C<20 se usa la binomial exacta sobre los pares discordantes: X ~ Bin(B+C, 0.5).

**3. [Verdadero/Falso]** Con B+C < 20 se prefiere el método exacto (binomial); con B+C ≥ 20, la χ² (con corrección de Yates).

   - **Respuesta:** Verdadero
   - _Explicación:_ Esa es la regla de decisión del método.

**4. [Cálculo numérico]** Prueba de McNemar (datos pareados). Tabla 2×2: A=310, B=10, C=35, D=205. Como B+C=45 ≥ 20, usa χ² con corrección de Yates: (|B−C|−1)²/(B+C).

   - **Respuesta:** 12.8
   - _Procedimiento:_ |B−C| = |10−35| = 25 → χ² = (|B−C|−1)²/(B+C) = (25−1)²/45 = 12.8
   - _Explicación:_ Solo los discordantes B y C aportan evidencia. Yates: (|B−C|−1)²/(B+C), con gl=1.

**5. [Relacionar]** En la tabla 2×2 de McNemar (antes → después):

   - **Respuesta:** A ↔ éxito → éxito · B ↔ éxito → fracaso · C ↔ fracaso → éxito · D ↔ fracaso → fracaso

**6. [Completar]** Los grados de libertad de la χ² de McNemar son ____ .
...
**2. [Cálculo numérico]** Prueba de Cox-Stuart de tendencia. Serie cronológica: {58, 55, 53, 49, 47, 43, 42, 37, 36, 33, 28, 23}. Se forman 6 parejas (xᵢ, xᵢ₊6). Con C=6 parejas efectivas y T ~ Bin(6, 0.5), calcula P(T ≤ 0) (cola del menor número de signos).
...
**3. [Cálculo numérico]** Si las 6 parejas de Cox-Stuart muestran todas disminución (0 signos «+»), el valor p de esa cola es P(T ≤ 0) = (0.5)⁶. Calcula ese valor.

   - **Respuesta:** 0.015625
   - _Procedimiento:_ (0.5)⁶ = 0.015625
   - _Explicación:_ (0.5)⁶ = 1/64 = 0.015625.

**4. [Opción múltiple]** La prueba de Cox-Stuart sirve para detectar:
...
**6. [Cálculo numérico]** Prueba de Cox-Stuart de tendencia. Serie cronológica: {77, 75, 73, 70, 66, 64, 63, 60, 57, 55}. Se forman 5 parejas (xᵢ, xᵢ₊5). Con C=5 parejas efectivas y T ~ Bin(5, 0.5), calcula P(T ≤ 0) (cola del menor número de signos).
...
**1. [Opción múltiple]** La prueba de Mann-Whitney es la alternativa no paramétrica a:
...
**2. [Cálculo numérico]** Mann-Whitney con Grupo A={13, 16, 18, 19, 22} (n₁=5) y Grupo B={25, 28, 31, 33} (n₂=4). Al ordenar todo, la suma de rangos de A es R₁=15. Calcula U = mín(U₁, U₂) con U₁=n₁n₂+n₁(n₁+1)/2−R₁ y U₂=n₁n₂−U₁.
...
**3. [Cálculo numérico]** Correlación de Spearman con n=6 y Σdᵢ²=0 (dᵢ = Rxᵢ − Ryᵢ). Calcula ρs = 1 − 6Σdᵢ² / [n(n²−1)].

   - **Respuesta:** 1
   - _Procedimiento:_ n(n²−1) = 6·(6²−1) = 210 → ρs = 1 − 6·0/210 = 1
   - _Explicación:_ ρs = 1 − 6Σdᵢ²/[n(n²−1)]. Signo + relación creciente, − decreciente.

**4. [Cálculo numérico]** Con n=7 y Σdᵢ²=40, aplica ρs = 1 − 6Σdᵢ²/[n(n²−1)].

   - **Respuesta:** 0.2857
   - _Procedimiento:_ ρs = 1 − 6(40)/[7(7²−1)] = 1 − 240/336 = 0.2857
   - _Explicación:_ Sustitución directa en la fórmula de Spearman.

**5. [Opción múltiple]** En Mann-Whitney se rechaza H₀ cuando:
...
**7. [Cálculo numérico]** Mann-Whitney con Grupo A={15, 17, 20, 21} (n₁=4) y Grupo B={22, 25, 27, 29, 31} (n₂=5). Al ordenar todo, la suma de rangos de A es R₁=10. Calcula U = mín(U₁, U₂) con U₁=n₁n₂+n₁(n₁+1)/2−R₁ y U₂=n₁n₂−U₁.

   - **Respuesta:** 0
   - _Procedimiento:_ U₁ = 4·5 + 4·5/2 − 10 = 20 → U₂ = 4·5 − 20 = 0 → U = mín(20, 0) = 0
   - _Explicación:_ U₁=n₁n₂+n₁(n₁+1)/2−R₁; U₂=n₁n₂−U₁; U=mín(U₁,U₂). Se rechaza H₀ si U ≤ U_crítico.

**8. [Cálculo numérico]** Mann-Whitney con Grupo A={14, 16, 17, 19} (n₁=4) y Grupo B={20, 21, 24, 25} (n₂=4). Al ordenar todo, la suma de rangos de A es R₁=10. Calcula U = mín(U₁, U₂) con U₁=n₁n₂+n₁(n₁+1)/2−R₁ y U₂=n₁n₂−U₁.

   - **Respuesta:** 0
   - _Procedimiento:_ U₁ = 4·4 + 4·5/2 − 10 = 16 → U₂ = 4·4 − 16 = 0 → U = mín(16, 0) = 0
   - _Explicación:_ U₁=n₁n₂+n₁(n₁+1)/2−R₁; U₂=n₁n₂−U₁; U=mín(U₁,U₂). Se rechaza H₀ si U ≤ U_crítico.

**9. [Cálculo numérico]** Mann-Whitney con Grupo A={13, 16, 17, 20, 21} (n₁=5) y Grupo B={22, 23, 26, 29, 32} (n₂=5). Al ordenar todo, la suma de rangos de A es R₁=15. Calcula U = mín(U₁, U₂) con U₁=n₁n₂+n₁(n₁+1)/2−R₁ y U₂=n₁n₂−U₁.
